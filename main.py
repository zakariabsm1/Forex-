...
API_RATE_LIMIT = int(os.getenv("API_RATE_LIMIT", "60"))  # req/min
...
async def fetch_oanda_candles(symbol: str, timeframe: str, count: int = 300) -> pd.DataFrame:
    """جلب بيانات حقيقية من OANDA"""
    tf_map = {"M15": "M15", "H1": "H1", "H4": "H4", "D1": "D1"}
    gran = tf_map.get(timeframe, "H1")

    oanda_symbol = symbol.replace("XAU", "XAU_").replace("USD", "USD") if "XAU" in symbol else f"{symbol[:3]}_{symbol[3:]}"
    ...
    try:
        ...
        logger.info(f"📊 OANDA: {len(df)} شمعة لـ {symbol}")
        return df

    except Exception as e:
        logger.warning(f"⚠️ OANDA API خطأ: {e} - استخدام بيانات تجريبية")
        return _generate_demo_data(symbol, count)


def _generate_demo_data(symbol: str, count: int = 300) -> pd.DataFrame:
    """بيانات تجريبية لاختبار النظام"""
    np.random.seed(42)
    base = 2000.0 if "XAU" in symbol else 1.1000

    # محاكاة سعر واقعي بـ Random Walk
    returns = np.random.normal(0.0001, 0.002, count)
    prices = base * np.exp(np.cumsum(returns))

    highs = prices * (1 + np.abs(np.random.normal(0, 0.001, count)))
    lows  = prices * (1 - np.abs(np.random.normal(0, 0.001, count)))
    ...
    timestamps = [start + timedelta(hours=i) for i in range(count)]
...
async def generate_signal(
    ...
    # جلب البيانات
    df = await fetch_oanda_candles(req.symbol, req.timeframe)
...
async def analyze_symbol(symbol: str, timeframe: str = "H1"):
    ...
    df = await fetch_oanda_candles(symbol, timeframe)
